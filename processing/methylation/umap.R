...
# Read SE and ADNI
se <- readRDS('~/Projects/CoMeT/data/ADNI/methylation/methylation.rds')
se <- cleanMatrixForClusterSE(se)
...
coldata <- cbind(umapResult2, as_tibble(coldata[samples,]))
rids <- unique(coldata$RID)
adniCols <- adni[adni$RID %in% rids,c('RID','DX_nearest_1.0')]
coldata <- left_join(coldata,adniCols,by='RID')

# Rename some columns
coldata <- coldata %>%
  rename(Diagnosis = DX_nearest_1.0,
         Plate = PlateNumber)
...
p1 <- ggplot(coldata,aes(x=UMAP1,y=UMAP2,color=Sex)) + geom_point() +
  labs(title='Sex')
p2 <- ggplot(coldata,aes(x=UMAP1,y=UMAP2,color=Array)) + geom_point() +
  labs(title='Array')
p3 <- ggplot(coldata,aes(x=UMAP1,y=UMAP2,color=Plate)) + geom_point() + guides(colour = guide_legend(nrow = 10)) +
  labs(title='Plate')
p4 <- ggplot(coldata,aes(x=UMAP1,y=UMAP2,color=Diagnosis)) + geom_point() + scale_color_hue(direction = -1) +
  labs(title='Clinical Diagnosis Group')
