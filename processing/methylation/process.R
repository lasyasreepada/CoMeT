...
rownames(adni_colData) <- adni_colData$Sample

# Checkpoint
identical(colnames(betas), rownames(adni_colData))

# Save as SummarizedExperiment
se <- SummarizedExperiment(assays = betas, colData=adni_colData, metadata = adni_qcs)

# Save to RDS file
saveRDS(se, "~/Projects/CoMeT/data/ADNI/methylation/methylation.rds")
