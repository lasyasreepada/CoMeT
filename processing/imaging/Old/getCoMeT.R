...
for (roi in cog) {
  ...
  # Fit regression to controls
  formula <- as.formula(paste0(roi,'~ AGE + PTGENDER_M + PTEDUCAT'))
  lmem <- lm(formula, data = subset(adniX,Case==0))
...
for (roi in c(rois.z,signatures.z)) {
  ...
  name_ti <- paste0(gsub('_Zscore','',roi),'_CoMeT')
  adniX[name_ti] <- ti
}
...
for (score in cog.z) {
  ...
  name_ti <- paste0(gsub('_Zscore','',score),'_CoMeT')
  adniX[name_ti] <- ti
}

# Save file 
write.csv(adniX,'data/ADNI/MRI_DNA_COG_XSH_COMET.csv')
